...
#define DTOR(d) d*M_PI/180 // Degree to Radians
...
class VFH_Algorithm
{
    ...
    float CAR_RADIUS; // Maximum distance of LiDAR from Car Edge
    int CENTER_X; //todo
    int CENTER_Y; //todo
    int HIST_SIZE; // sectors

    float CELL_WIDTH; // Cells of Histogram (mm)
    int WINDOW_DIAMETER; // Number of cells
    int SECTOR_ANGLE; //Degrees
    float SAFETY_DIST_0MS; // mm Distance from Obstacle 
    float SAFETY_DIST_1MS; // mm Distance from Obstacle
    ...
    //* You're returning them by reference
    // Units/Senses:
    //  - goal_direction in degrees, 0deg is to the right. //! Check this out
    //  - goal_distance  in mm.
